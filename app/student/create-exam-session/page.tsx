...
export default function CreateExamSessionPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (bloomLevel.length === 0) {
      alert('Vui lòng chọn ít nhất một mức độ khó Bloom');
      return;
    }
    
    // Demo-only: just show alert and redirect
    alert('Đã tạo buổi luyện tập vấn đáp (Demo mode)');
    router.push('/student/create-session');
  };
      ...
      <main className="max-w-3xl mx-auto px-5 py-10">
        ...
        <form onSubmit={handleSubmit} className="bg-white  shadow-sm p-8 space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              type="submit"
              className="flex-1 px-6 py-2.5 bg-[#0065ca] text-white font-semibold  hover:bg-[#004a95] transition-colors"
            >
              Tạo buổi luyện tập
            </button>
          </div>
